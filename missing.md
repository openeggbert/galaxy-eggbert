...
`GalaxyEggbertCNA`'s `TileAtlas` was written, it independently reimplemented tile UV lookup via
...
inset. `TileAtlas.cpp`'s own comment even cited `tileUV()`'s doc comment as the source of the
...
**Fix:** `TileAtlas::GetTileUv()` now calls `BlockTypes::tileUV()` directly instead of its own
