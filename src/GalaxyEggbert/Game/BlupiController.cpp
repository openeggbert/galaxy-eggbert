...
namespace GalaxyEggbert::Game
{
    namespace
    {
        constexpr int kWorldCenterX = WorldRuntime::kWorldCenterX;
        constexpr int kWorldCenterZ = WorldRuntime::kWorldCenterZ;
    ...
    void BlupiController::RideLift(float x, float y, float z) noexcept
    {
        m_x = x;
        m_y = y;
        m_z = z;
    ...
    bool BlupiController::IsSolidAt(const Worlds::World& world, int gx, int gy, int gz)
    {
    ...
    bool BlupiController::IsPointSolid(const Worlds::World& world, float x, float y, float z, bool tempPassable,
                                          bool checkSubcell)
    {
    ...
    void BlupiController::RecoverFromPenetration(const Worlds::World& world, float& x, float& y, float& z,
                                                    bool tempPassable)
    {
    ...
    bool BlupiController::HasJumpHeadroom(const Worlds::World& world) const
    {
        const int blocksPerAxis = static_cast<int>(world.blocksPerAxis());
    ...
    BlupiController::BarreCellType BlupiController::GetBarreCellType(const Worlds::World& world,
    ...
    bool BlupiController::TriggerCrush() noexcept
    {
        if (m_ecrase)
        {
            return false;
    ...
    bool BlupiController::TriggerBalloon() noexcept
    {
        if (m_balloon)
        {
            return false;
    ...
    void BlupiController::PopBalloon() noexcept
    {
        if (!m_balloon)
        {
            return;
    ...
    bool BlupiController::TriggerShield() noexcept
    {
    ...
    bool BlupiController::TriggerPower() noexcept
    {
    ...
    bool BlupiController::TriggerCloud() noexcept
    {
    ...
    bool BlupiController::TriggerHide() noexcept
    {
    ...
    bool BlupiController::TriggerInvert() noexcept
    {
    ...
    bool BlupiController::TriggerMount(VehicleMode mode, bool inNage, bool inSurf) noexcept
    {
    ...
    void BlupiController::TriggerDismount() noexcept
    {
        if (!IsInVehicle())
        {
            return;
    ...
    bool BlupiController::ToggleGhost(const Worlds::World& world) noexcept
    {
        if (!m_ghost)
        {
            m_ghost = true;
    ...
    bool BlupiController::TriggerSpringBounce(bool jumpHeld) noexcept
    {
        if (!m_onGround)
        {
            return false;
    ...
    bool BlupiController::TriggerTeleport(std::uint16_t icon) noexcept
    {
    ...
    bool BlupiController::TriggerBye() noexcept
    {
    ...
    bool BlupiController::TriggerOneShotAnim(AnimState state, float durationSeconds) noexcept
    {
    ...
    bool BlupiController::TriggerMockery(AnimState variant) noexcept
    {
    ...
    bool BlupiController::TriggerDeathLock(DeathCause cause, bool shouldRespawn) noexcept
    {
        ...
        // InteractionSystem.cpp's own comment). This engine's death-lock previously left every
    ...
    bool BlupiController::TriggerPickupFreeze(PickupFreezeKind kind) noexcept
    {
    ...
    bool BlupiController::ConsumePickupFreezeResolved(PickupFreezeKind& outKind) noexcept
    {
    ...
    bool BlupiController::ConsumeDeathLockResolved(bool& outShouldRespawn) noexcept
    {
    ...
    void BlupiController::UpdateSafePosition(bool externallySafe) noexcept
    {
    ...
    std::uint16_t BlupiController::GetGroundBlockType(const Worlds::World& world) const noexcept
    {
        if (!m_onGround)
        {
    ...
    std::uint16_t BlupiController::GetBlockTypeAbove(const Worlds::World& world) const noexcept
    {
        const int blocksPerAxis = static_cast<int>(world.blocksPerAxis());
        const int gx = ClampGrid(static_cast<int>(std::lround(m_x + kWorldCenterX)), blocksPerAxis);
        const int gz = ClampGrid(static_cast<int>(std::lround(m_z + kWorldCenterZ)), blocksPerAxis);
    ...
    std::uint16_t BlupiController::GetBlockTypeAt(const Worlds::World& world) const noexcept
    {
        const int blocksPerAxis = static_cast<int>(world.blocksPerAxis());
        const int gx = ClampGrid(static_cast<int>(std::lround(m_x + kWorldCenterX)), blocksPerAxis);
        const int gz = ClampGrid(static_cast<int>(std::lround(m_z + kWorldCenterZ)), blocksPerAxis);
    ...
    float BlupiController::FindColumnGroundY(const Worlds::World& world, float x, float z, float referenceY,
                                                bool tempPassable)
    {
        const int blocksPerAxis = static_cast<int>(world.blocksPerAxis());
    ...
    void BlupiController::Step(const Worlds::World& world, float turnInput, float moveInput,
    ...
    void BlupiController::UpdateAnim(bool moving, bool crouchHeld, bool lookUpHeld, float dt, bool pushingCrate)
    {
    ...
    int BlupiController::GetAnimIcon() const noexcept
    {
        switch (m_animState)
        {
    ...
    bool BlupiController::AnimIconUsesElementSheet() const noexcept
    {
    ...
    int BlupiController::GetDisplayAnimIcon() const noexcept
    {
        // Real table_mirror (Tables.cpp:413-447), transcribed verbatim.
        static const int kMirror[335] = {
        ...
        const int icon = GetAnimIcon();
        bool facingLeft = std::sin(m_yaw) < 0.0f;
        if (m_invert)
        {
        ...
        {
            return (icon >= 0 && icon < 335) ? kMirror[icon] : icon;
        }
        // Real extra +4 special case for element.png icons 168-171 while
        // facing left (Decor.cpp:2446-2449) -- Glu's own real frame table
        // (kGluFrames, 168-171) is exactly this range.
        return (icon >= 168 && icon <= 171) ? icon + 4 : icon;
    }
}
