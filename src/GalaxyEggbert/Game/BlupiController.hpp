...
namespace GalaxyEggbert::Game
{
    ...
    public:
        ...
        // BlupiController.cpp) are a fresh, narrowly-scoped transcription
        ...
        enum class AnimState : std::uint8_t
        {
            ...
            // true (the caller's per-frame `InteractionSystem::
            ...
            // caller alongside `InteractionSystem::EventKind::TankFired`
        ...
        // `InputPad::UpdateTypedGhostCheat()`'s own comment. Real
        ...
        // class has no access to WorldRuntime/MobileObjSpec.
        ...
        // WorldRuntime::FindTeleportDestination() once IsTeleporting()
        ...
        //      `m_blupiRestart`-gated respawn NOW, call `InteractionSystem::LoseLife()`, and
        //      start the cosmetic icon-48 life-loss Voyage itself (this class has no dependency on
        //      InteractionSystem/Sound, same one-way layering as the lift-riding
        ...
        // speed=0, static) -- the caller does this via InteractionSystem, this class has no world
        ...
        // pre-computed WorldRuntime::IsTempPassableAtPhase() result for
        // this frame -- kept as a plain bool rather than a WorldRuntime
        ...
        // per-frame `InteractionSystem::CrateBeingPushedThisFrame()`
    ...
    private:
        ...
        // (DecorQuartTable.hpp, Tables::table_decor_quart, explicit user
