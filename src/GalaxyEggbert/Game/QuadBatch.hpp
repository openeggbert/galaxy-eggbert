...
namespace GalaxyEggbert::Game::QuadBatch
{
    ...
    // InputPad (plan.md EDITOR-106) so the editor's own UI (palette,
    ...
    // same shape InputPad's own private helpers already had.

    // Axis-aligned rect in whatever 2D space the caller is working in
    // (InputPad uses its own fixed 640x480 reference space; the editor
    ...
    // uniform, same as InputPad's original shape).
    struct Quad
    {
        float x0, y0, x1, y1;
        float u0, v0, u1, v1;
