...
not repeated here): `DOC-100`–`DOC-116` done** (root cause + translucency fix + `tileUV` engine
bug + all 12 tile animations + first 4 Blupi actions). 113 of the 129 GIFs (`DOC-117`–`DOC-229`)
still need regenerating — next is `DOC-117` (`blupi-action-05-air.gif`).
...
| confirmed, documentation only, tool now fixed | 113 of 129 animated GIFs in `mobile-eggbert-reference/images/` still ghost/accumulate previous frames instead of clearing (confirmed via alpha-channel analysis on coalesced frames — see §4). Root cause fixed (`DOC-100`) plus a second tool fix for translucent content vanishing (`DOC-105`); `DOC-100`–`DOC-116` regenerated and verified so far (see `plan.md` §16 for per-task detail) — the other 113 (`DOC-117`–`DOC-229`) still need regenerating with the fixed tool. |
...
1. **Work through the ~168-task `mobile-eggbert-reference/` rework list (in progress, reopened
   2026-07-03 — see §4 and `plan.md` §16 for the full story).** `DOC-100`–`DOC-116` are done: the
   ...
   `S3D-2`), all 12 tile animations, and the first 4 of 84 Blupi actions (also landed a reusable
   ...
   in `plan.md` §16, not repeated here. Next: `DOC-117` (`blupi-action-05-air.gif`), then continue
   through `DOC-118`–`DOC-267` (112 more GIF regenerations, then static-icon re-verification, then
