...
| confirmed, documentation only, tool now fixed | 8 of 129 animated GIFs in `mobile-eggbert-reference/images/` still ghost/accumulate previous frames instead of clearing (confirmed via alpha-channel analysis on coalesced frames — see §4). Root cause fixed (`DOC-100`) plus a second tool fix for translucent content vanishing (`DOC-105`); `DOC-100`–`DOC-221` regenerated and verified so far (see `plan.md` §16 for per-task detail) — the other 8 (`DOC-222`–`DOC-229`) still need regenerating with the fixed tool. |
...
1. **Work through the ~168-task `mobile-eggbert-reference/` rework list (in progress, reopened
   ...
   originally confirmed the `DOC-100` ghosting bug report, now confirmed fixed) is also done.
   Next: `DOC-222`, using `explo.png` + mobile-eggbert's `table_explo2` (galaxy-eggbert has only
   ported `table_explo1` so far; `table_explo2`-`table_explo8` are read live from
