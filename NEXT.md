...
follow-up work through EDITOR-125 is also complete, including the Eggbert-ordered palette cleanup,
XYZ placement controls/readout, object cell alignment, the Galaxy-only background-thumbnail menu,
functional touch deletion, all Cave/Buildings and special-object mappings, and persisted Level start. Editor work resumed
...
editor work is a separately selected follow-up. The user has now selected `EDITOR-126` and
`EDITOR-127`: complete the 8 source-menu entries that still show the temporary notice through
the required BigDecor persistence and exhaustive final verification._
...
follow-ups through `EDITOR-125` are implemented — see plan.md §6 for the full write-up (what's
...
editor plan is fully complete, and user-directed follow-ups through `EDITOR-125` are implemented.
...
the user-directed follow-ups through `EDITOR-125` are implemented. Further editor work is selected
...
4. **EDITOR-123 through EDITOR-127 — complete all remaining source-menu entries** — **active,
   explicitly approved 2026-07-25.** EDITOR-123's Cave/Buildings, EDITOR-124's special objects,
   and EDITOR-125's persisted Level start are done. The remaining 8-item inventory and acceptance
   criteria are in `plan.md` §6: editable/persisted BigDecor scenery and exhaustive verification.
