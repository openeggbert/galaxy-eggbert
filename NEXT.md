...
fluctuates instead of climbing) and `DOC-115` (`blupi-action-03-turn.gif`) are also done. 114 of
the 129 GIFs (`DOC-116`–`DOC-229`) are still not regenerated — next is `DOC-116`
(`blupi-action-04-jump.gif`).
...
| confirmed, documentation only, tool now fixed | 114 of 129 animated GIFs in `mobile-eggbert-reference/images/` still ghost/accumulate previous frames instead of clearing (confirmed via alpha-channel analysis on coalesced frames — see §4). Root-cause tool fix landed (`DOC-100`), plus a second tool fix for translucent content vanishing (found via `DOC-105`). **All 12 tile animations regenerated and verified** (`DOC-101`-`DOC-112`), plus `DOC-113`-`DOC-115` (first 3 of 84 Blupi actions — `DOC-114` is the exact GIF that originally surfaced the `DOC-100` bug report, now confirmed fixed); the other 114 (`DOC-116`–`DOC-229`) still need regenerating with the fixed tool. |
...
1. **Work through the ~168-task `mobile-eggbert-reference/` rework list (in progress, reopened
   ...
   mobile-eggbert's `table_blupi` live rather than copying it) and `DOC-114`
   (`blupi-action-02-march.gif` — the exact GIF that originally surfaced the `DOC-100` bug, now
   confirmed fixed) and `DOC-115` (`blupi-action-03-turn.gif`) are done. Next: `DOC-116`
   (`blupi-action-04-jump.gif`), then continue through
   `DOC-117`–`DOC-267` (113 more GIF
