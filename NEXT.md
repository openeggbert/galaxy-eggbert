...
not repeated here): `DOC-100`–`DOC-186` done** (root cause + translucency fix + `tileUV` engine
bug + all 12 tile animations + 74 Blupi actions (3 of the 87 real BlupiActions -- 12/Set,
...
per-frame blupi.png/element.png channel switch. 43 of the 129 GIFs (`DOC-187`–`DOC-229`)
still need regenerating — next is `DOC-187` (`blupi-action-78-clear5.gif`).
...
| confirmed, documentation only, tool now fixed | 43 of 129 animated GIFs in `mobile-eggbert-reference/images/` still ghost/accumulate previous frames instead of clearing (confirmed via alpha-channel analysis on coalesced frames — see §4). Root cause fixed (`DOC-100`) plus a second tool fix for translucent content vanishing (`DOC-105`); `DOC-100`–`DOC-186` regenerated and verified so far (see `plan.md` §16 for per-task detail) — the other 43 (`DOC-187`–`DOC-229`) still need regenerating with the fixed tool. |
...
1. **Work through the ~168-task `mobile-eggbert-reference/` rework list (in progress, reopened
   2026-07-03 — see §4 and `plan.md` §16 for the full story).** `DOC-100`–`DOC-186` are done: the
   ...
   `S3D-2`), all 12 tile animations, and 74 of 84 Blupi actions (also landed a reusable
   ...
   in `plan.md` §16, not repeated here. Next: `DOC-187` (`blupi-action-78-clear5.gif`), then continue
   through `DOC-188`–`DOC-267` (42 more GIF regenerations, then static-icon re-verification, then
