...
| confirmed, documentation only, tool now fixed | 5 of 129 animated GIFs in `mobile-eggbert-reference/images/` still ghost/accumulate previous frames instead of clearing (confirmed via alpha-channel analysis on coalesced frames — see §4). Root cause fixed (`DOC-100`) plus a second tool fix for translucent content vanishing (`DOC-105`); `DOC-100`–`DOC-224` regenerated and verified so far (see `plan.md` §16 for per-task detail) — the other 5 (`DOC-225`–`DOC-229`) still need regenerating with the fixed tool. |
...
1. **Work through the ~168-task `mobile-eggbert-reference/` rework list (in progress, reopened
   ...
   (`explosion-anim-explo2.gif`), `DOC-223` (`explosion-anim-explo3.gif`), and `DOC-224`
   (`explosion-anim-explo4.gif`) are also done. Next: `DOC-225`, using `explo.png` +
   mobile-eggbert's `table_explo5` (galaxy-eggbert has only ported `table_explo1` so far;
   `table_explo2`-`table_explo8` are read live from
