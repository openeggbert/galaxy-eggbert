...
`DOC-212` (`object-anim-type40-mirrorinvert.gif`) are now also done, next is `DOC-213`
(`object-anim-type44-wasp.gif`).
...
| confirmed, documentation only, tool now fixed | 17 of 129 animated GIFs in `mobile-eggbert-reference/images/` still ghost/accumulate previous frames instead of clearing (confirmed via alpha-channel analysis on coalesced frames — see §4). Root cause fixed (`DOC-100`) plus a second tool fix for translucent content vanishing (`DOC-105`); `DOC-100`–`DOC-212` regenerated and verified so far (see `plan.md` §16 for per-task detail) — the other 17 (`DOC-213`–`DOC-229`) still need regenerating with the fixed tool. |
