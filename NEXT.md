...
| confirmed, documentation only, tool now fixed | 7 of 129 animated GIFs in `mobile-eggbert-reference/images/` still ghost/accumulate previous frames instead of clearing (confirmed via alpha-channel analysis on coalesced frames — see §4). Root cause fixed (`DOC-100`) plus a second tool fix for translucent content vanishing (`DOC-105`); `DOC-100`–`DOC-222` regenerated and verified so far (see `plan.md` §16 for per-task detail) — the other 7 (`DOC-223`–`DOC-229`) still need regenerating with the fixed tool. |
...
1. **Work through the ~168-task `mobile-eggbert-reference/` rework list (in progress, reopened
   ...
   originally confirmed the `DOC-100` ghosting bug report, now confirmed fixed) and `DOC-222`
   (`explosion-anim-explo2.gif`) are also done. Next: `DOC-223`, using `explo.png` + mobile-eggbert's
   `table_explo3` (galaxy-eggbert has only ported `table_explo1` so far; `table_explo2`-`table_explo8`
   are read live from `../mobile-eggbert/src/WindowsPhoneSpeedyBlupi/Tables.cpp` for these doc
   tasks only, not
