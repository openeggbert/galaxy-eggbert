...
not repeated here): `DOC-100`–`DOC-120` done** (root cause + translucency fix + `tileUV` engine
bug + all 12 tile animations + first 8 Blupi actions). 109 of the 129 GIFs (`DOC-121`–`DOC-229`)
still need regenerating — next is `DOC-121` (`blupi-action-09-recede.gif`).
...
| confirmed, documentation only, tool now fixed | 109 of 129 animated GIFs in `mobile-eggbert-reference/images/` still ghost/accumulate previous frames instead of clearing (confirmed via alpha-channel analysis on coalesced frames — see §4). Root cause fixed (`DOC-100`) plus a second tool fix for translucent content vanishing (`DOC-105`); `DOC-100`–`DOC-120` regenerated and verified so far (see `plan.md` §16 for per-task detail) — the other 109 (`DOC-121`–`DOC-229`) still need regenerating with the fixed tool. |
...
1. **Work through the ~168-task `mobile-eggbert-reference/` rework list (in progress, reopened
   2026-07-03 — see §4 and `plan.md` §16 for the full story).** `DOC-100`–`DOC-120` are done: the
   ...
   `S3D-2`), all 12 tile animations, and the first 8 of 84 Blupi actions (also landed a reusable
   ...
   in `plan.md` §16, not repeated here. Next: `DOC-121` (`blupi-action-09-recede.gif`), then continue
   through `DOC-122`–`DOC-267` (108 more GIF regenerations, then static-icon re-verification, then
