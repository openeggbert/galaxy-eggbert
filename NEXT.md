...
`DOC-208` (`object-anim-type25-shield.gif`) are now also done, next is `DOC-209`
(`object-anim-type26-suctioncup.gif`).
...
| confirmed, documentation only, tool now fixed | 21 of 129 animated GIFs in `mobile-eggbert-reference/images/` still ghost/accumulate previous frames instead of clearing (confirmed via alpha-channel analysis on coalesced frames — see §4). Root cause fixed (`DOC-100`) plus a second tool fix for translucent content vanishing (`DOC-105`); `DOC-100`–`DOC-208` regenerated and verified so far (see `plan.md` §16 for per-task detail) — the other 21 (`DOC-209`–`DOC-229`) still need regenerating with the fixed tool. |
