...
false alarm, see `plan.md`), and `DOC-107` (`tile-anim-temp.gif` — 18 real icons + 2 genuinely
blank `-1`-sentinel frames, all clean) are done. 122 of the 129 GIFs (`DOC-108`–`DOC-229`) are
still not regenerated — next is `DOC-108` (`tile-anim-marine.gif`).
...
| confirmed, documentation only, tool now fixed | 122 of 129 animated GIFs in `mobile-eggbert-reference/images/` still ghost/accumulate previous frames instead of clearing (confirmed via alpha-channel analysis on coalesced frames — see §4). Root-cause tool fix landed (`DOC-100`), plus a second tool fix for translucent content vanishing (found via `DOC-105`); `tile-anim-lava.gif`/`tile-anim-spike.gif`/`tile-anim-crusher.gif`/`tile-anim-saw.gif`/`tile-anim-water1.gif`/`tile-anim-water2.gif`/`tile-anim-temp.gif` regenerated (`DOC-101`-`DOC-107`); the other 122 (`DOC-108`–`DOC-229`) still need regenerating with the fixed tool. |
...
1. **Work through the ~168-task `mobile-eggbert-reference/` rework list (in progress, reopened
   ...
   content vanishing entirely — see §3), `DOC-106` (`tile-anim-water2.gif`), and `DOC-107`
   (`tile-anim-temp.gif`) are done. Next: `DOC-108`, regenerate + verify `tile-anim-marine.gif`
   with the fixed tool, then continue through `DOC-109`–`DOC-267` (121 more GIF regenerations,
