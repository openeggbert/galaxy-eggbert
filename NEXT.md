...
not repeated here): `DOC-100`–`DOC-189` done** (root cause + translucency fix + `tileUV` engine
bug + all 12 tile animations + 77 Blupi actions (3 of the 87 real BlupiActions -- 12/Set,
70, 71 -- confirmed to have no table_blupi entry, need no GIF); `DOC-184`/`DOC-187`/`DOC-188`/
`DOC-189` (`Clear2`/`Clear5`/`Clear6`/`Clear7`) are genuinely blank single-frame animations,
not bugs; `DOC-168` is the only action with a real per-frame blupi.png/element.png channel
switch. 40 of the 129 GIFs (`DOC-190`–`DOC-229`) still need regenerating — next is `DOC-190`
(`blupi-action-81-clear8.gif`).
...
| confirmed, documentation only, tool now fixed | 40 of 129 animated GIFs in `mobile-eggbert-reference/images/` still ghost/accumulate previous frames instead of clearing (confirmed via alpha-channel analysis on coalesced frames — see §4). Root cause fixed (`DOC-100`) plus a second tool fix for translucent content vanishing (`DOC-105`); `DOC-100`–`DOC-189` regenerated and verified so far (see `plan.md` §16 for per-task detail) — the other 40 (`DOC-190`–`DOC-229`) still need regenerating with the fixed tool. |
...
1. **Work through the ~168-task `mobile-eggbert-reference/` rework list (in progress, reopened
   2026-07-03 — see §4 and `plan.md` §16 for the full story).** `DOC-100`–`DOC-189` are done: the
   ...
   `S3D-2`), all 12 tile animations, and 77 of 84 Blupi actions (also landed a reusable
   ...
   in `plan.md` §16, not repeated here. Next: `DOC-190` (`blupi-action-81-clear8.gif`), then continue
   through `DOC-191`–`DOC-267` (39 more GIF regenerations, then static-icon re-verification, then
