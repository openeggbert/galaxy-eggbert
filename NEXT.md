...
and `DOC-199` (`object-anim-type04-bulldozer.gif`) are also done. 30 of the 129 GIFs
(`DOC-200`–`DOC-229`) still need regenerating — next is `DOC-200`
(`object-anim-type05-treasure.gif` — this is the exact GIF that first surfaced the
translucent-content-vanishing bug found via `DOC-105`).
...
| confirmed, documentation only, tool now fixed | 30 of 129 animated GIFs in `mobile-eggbert-reference/images/` still ghost/accumulate previous frames instead of clearing (confirmed via alpha-channel analysis on coalesced frames — see §4). Root cause fixed (`DOC-100`) plus a second tool fix for translucent content vanishing (`DOC-105`); `DOC-100`–`DOC-199` regenerated and verified so far (see `plan.md` §16 for per-task detail) — the other 30 (`DOC-200`–`DOC-229`) still need regenerating with the fixed tool. |
...
1. **Work through the ~168-task `mobile-eggbert-reference/` rework list (in progress, reopened
   ...
   (`object-anim-type03-patrolB.gif`), and `DOC-199` (`object-anim-type04-bulldozer.gif`) are
   done. Next: `DOC-200` (`object-anim-type05-treasure.gif`), then continue through
   `DOC-201`–`DOC-267` (29 more GIF regenerations, then static-icon re-verification, then
