...
follow-up work through EDITOR-124 is also complete, including the Eggbert-ordered palette cleanup,
XYZ placement controls/readout, object cell alignment, the Galaxy-only background-thumbnail menu,
functional touch deletion, all Cave/Buildings mappings, and all six special-object mappings. Editor work resumed
...
editor work is a separately selected follow-up. The user has now selected `EDITOR-125` through
`EDITOR-127`: complete the 9 source-menu entries that still show the temporary notice, including
...
follow-ups through `EDITOR-124` are implemented — see plan.md §6 for the full write-up (what's
...
editor plan is fully complete, and user-directed follow-ups through `EDITOR-124` are implemented.
...
the user-directed follow-ups through `EDITOR-124` are implemented. Further editor work is selected
...
4. **EDITOR-123 through EDITOR-127 — complete all remaining source-menu entries** — **active,
   explicitly approved 2026-07-25.** EDITOR-123's Cave/Buildings and EDITOR-124's six special
   objects are done. The remaining 9-item inventory and acceptance criteria are in `plan.md` §6:
