...
not repeated here): `DOC-100`–`DOC-164` done** (root cause + translucency fix + `tileUV` engine
bug + all 12 tile animations + 52 Blupi actions, one of which -- action 12/Set -- has no
table_blupi entry and needs no GIF). 65 of the 129 GIFs (`DOC-165`–`DOC-229`)
still need regenerating — next is `DOC-165` (`blupi-action-54-glu.gif`).
...
| confirmed, documentation only, tool now fixed | 65 of 129 animated GIFs in `mobile-eggbert-reference/images/` still ghost/accumulate previous frames instead of clearing (confirmed via alpha-channel analysis on coalesced frames — see §4). Root cause fixed (`DOC-100`) plus a second tool fix for translucent content vanishing (`DOC-105`); `DOC-100`–`DOC-164` regenerated and verified so far (see `plan.md` §16 for per-task detail) — the other 65 (`DOC-165`–`DOC-229`) still need regenerating with the fixed tool. |
...
1. **Work through the ~168-task `mobile-eggbert-reference/` rework list (in progress, reopened
   2026-07-03 — see §4 and `plan.md` §16 for the full story).** `DOC-100`–`DOC-164` are done: the
   ...
   `S3D-2`), all 12 tile animations, and 52 of 84 Blupi actions (also landed a reusable
   ...
   in `plan.md` §16, not repeated here. Next: `DOC-165` (`blupi-action-54-glu.gif`), then continue
   through `DOC-166`–`DOC-267` (64 more GIF regenerations, then static-icon re-verification, then
