...
follow-up work through EDITOR-126 is also complete, including the Eggbert-ordered palette cleanup,
XYZ placement controls/readout, object cell alignment, the Galaxy-only background-thumbnail menu,
functional touch deletion, all source mappings, persisted Level start, and editable BigDecor. Editor work resumed
...
editor work is a separately selected follow-up. The user has now selected `EDITOR-127`:
exhaustively verify every source-menu cell, remove any reachable temporary fallback, and visually
verify the final menu._
...
follow-ups through `EDITOR-126` are implemented — see plan.md §6 for the full write-up (what's
...
editor plan is fully complete, and user-directed follow-ups through `EDITOR-126` are implemented.
...
the user-directed follow-ups through `EDITOR-126` are implemented. Further editor work is selected
...
4. **EDITOR-123 through EDITOR-127 — complete all remaining source-menu entries** — **active,
   ...
   EDITOR-125's persisted Level start, and EDITOR-126's eight editable BigDecor entries are done.
   EDITOR-127's exhaustive menu/regression/visual acceptance criteria remain in `plan.md` §6.
