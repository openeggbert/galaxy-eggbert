...
`DOC-199` (`object-anim-type04-bulldozer.gif`), and `DOC-200` (`object-anim-type05-treasure.gif`
— the exact GIF that first surfaced the `DOC-100` ghosting bug report, now confirmed fixed) are
also done. 29 of the 129 GIFs (`DOC-201`–`DOC-229`) still need regenerating — next is `DOC-201`
(`object-anim-type06-egg.gif`).
...
| confirmed, documentation only, tool now fixed | 29 of 129 animated GIFs in `mobile-eggbert-reference/images/` still ghost/accumulate previous frames instead of clearing (confirmed via alpha-channel analysis on coalesced frames — see §4). Root cause fixed (`DOC-100`) plus a second tool fix for translucent content vanishing (`DOC-105`); `DOC-100`–`DOC-200` regenerated and verified so far (see `plan.md` §16 for per-task detail) — the other 29 (`DOC-201`–`DOC-229`) still need regenerating with the fixed tool. |
...
1. **Work through the ~168-task `mobile-eggbert-reference/` rework list (in progress, reopened
   ...
   (`object-anim-type03-patrolB.gif`), `DOC-199` (`object-anim-type04-bulldozer.gif`), and
   `DOC-200` (`object-anim-type05-treasure.gif` — the exact GIF that first surfaced the `DOC-100`
   ghosting bug, now confirmed fixed) are done. Next: `DOC-201` (`object-anim-type06-egg.gif`),
   then continue through `DOC-202`–`DOC-267` (28 more GIF regenerations, then static-icon
