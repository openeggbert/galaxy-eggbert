...
`-1`-sentinel frames, all clean), `DOC-108` (`tile-anim-marine.gif`), and `DOC-109`
(`tile-anim-fanleft.gif`) are done. 120 of the 129 GIFs (`DOC-110`–`DOC-229`) are still not
regenerated — next is `DOC-110` (`tile-anim-fanright.gif`).
...
| confirmed, documentation only, tool now fixed | 120 of 129 animated GIFs in `mobile-eggbert-reference/images/` still ghost/accumulate previous frames instead of clearing (confirmed via alpha-channel analysis on coalesced frames — see §4). Root-cause tool fix landed (`DOC-100`), plus a second tool fix for translucent content vanishing (found via `DOC-105`); `tile-anim-lava.gif`/`tile-anim-spike.gif`/`tile-anim-crusher.gif`/`tile-anim-saw.gif`/`tile-anim-water1.gif`/`tile-anim-water2.gif`/`tile-anim-temp.gif`/`tile-anim-marine.gif`/`tile-anim-fanleft.gif` regenerated (`DOC-101`-`DOC-109`); the other 120 (`DOC-110`–`DOC-229`) still need regenerating with the fixed tool. |
...
1. **Work through the ~168-task `mobile-eggbert-reference/` rework list (in progress, reopened
   ...
   (`tile-anim-temp.gif`), `DOC-108` (`tile-anim-marine.gif`), and `DOC-109`
   (`tile-anim-fanleft.gif`) are done. Next: `DOC-110`, regenerate + verify
   `tile-anim-fanright.gif` with the fixed tool, then continue through
   `DOC-111`–`DOC-267` (119 more GIF regenerations,
