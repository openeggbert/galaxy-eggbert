...
table_blupi entry, needed no GIF). 33 of the 129 GIFs (`DOC-197`–`DOC-229`) still need
regenerating — next is `DOC-197`, the first of 24 object/pickup/enemy animations
(`object-anim-type02-patrolA.gif`).
...
| confirmed, documentation only, tool now fixed | 33 of 129 animated GIFs in `mobile-eggbert-reference/images/` still ghost/accumulate previous frames instead of clearing (confirmed via alpha-channel analysis on coalesced frames — see §4). Root cause fixed (`DOC-100`) plus a second tool fix for translucent content vanishing (`DOC-105`); `DOC-100`–`DOC-196` regenerated and verified so far (see `plan.md` §16 for per-task detail) — the other 33 (`DOC-197`–`DOC-229`) still need regenerating with the fixed tool. |
...
1. **Work through the ~168-task `mobile-eggbert-reference/` rework list (in progress, reopened
   2026-07-03 — see §4 and `plan.md` §16 for the full story).** `DOC-100`–`DOC-196` are done: the
   ...
   `S3D-2`), all 12 tile animations, and all 84 Blupi actions (also landed a reusable
   ...
   in `plan.md` §16, not repeated here. **All 84 Blupi action GIFs are done.** Next: `DOC-197`, the first of 24
   object/pickup/enemy animations (`object-anim-type02-patrolA.gif`), then continue through
   `DOC-198`–`DOC-267` (32 more GIF regenerations, then static-icon re-verification, then
