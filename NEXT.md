...
`plan.md`'s `S3D-2`). `DOC-102` (`tile-anim-spike.gif`, high row indices 17-18 of 22) and `DOC-103`
(`tile-anim-crusher.gif`, last-column + row-wrap edge cases) are also done — no bleed/seam in
either. 126 of the 129 GIFs (`DOC-104`–`DOC-229`) are still not regenerated — next is `DOC-104`
(`tile-anim-saw.gif`).
...
| confirmed, documentation only, tool now fixed | 126 of 129 animated GIFs in `mobile-eggbert-reference/images/` still ghost/accumulate previous frames instead of clearing (confirmed via alpha-channel analysis on coalesced frames — see §4). Root-cause tool fix landed (`DOC-100`); `tile-anim-lava.gif`/`tile-anim-spike.gif`/`tile-anim-crusher.gif` regenerated (`DOC-101`-`DOC-103`); the other 126 (`DOC-104`–`DOC-229`) still need regenerating with the fixed tool. |
...
1. **Work through the ~168-task `mobile-eggbert-reference/` rework list (in progress, reopened
   ...
   (`tile-anim-spike.gif`), and `DOC-103` (`tile-anim-crusher.gif`) are done. Next: `DOC-104`,
   regenerate + verify `tile-anim-saw.gif` with the fixed tool and gap-aware crop coordinates,
   then continue through `DOC-105`–`DOC-267` (125 more GIF regenerations, then static-icon
   re-verification, then `DOC-005`/`DOC-006` sounds/backgrounds which were never
