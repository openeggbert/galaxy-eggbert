...
// spawns the 12 ObjectType values that were previously silently dropped by
// the "supported" allowlist (see mobile-eggbert-2d-reference.md §2.4). Each
// (type, file) pair below is a real mobile-eggbert level file confirmed (by
// grepping ../mobile-eggbert/worlds/*.txt) to contain a MoveObject: line of
// that exact type, so this is grounded in real level data, not synthetic.
int main()
{
